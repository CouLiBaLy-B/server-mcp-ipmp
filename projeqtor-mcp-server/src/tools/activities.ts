...
export function registerActivityTools(server: McpServer, client: ProjeqtorApiClient, log: Logger) {
  server.tool(
    'list_activities',
    ...
    withErrorHandling(async ({ idProject, status, type, responsibleId }) => {
      ...
      return { content: [{ type: 'text' as const, text: JSON.stringify({ count: filtered.length, activities: filtered }, null, 2) }] };
    }, 'list_activities'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async (args) => {
      ...
      return { content: [{ type: 'text' as const, text: `Activity created successfully:\n${JSON.stringify(result, null, 2)}` }] };
    }, 'create_activity'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async (args) => {
      ...
      return { content: [{ type: 'text' as const, text: `Activity updated successfully:\n${JSON.stringify(result, null, 2)}` }] };
    }, 'update_activity'),
  );

  server.tool(
    'get_gantt_data',
    'Retrieve Gantt chart data for a project. Returns all activities with their planned start/end dates, durations, dependencies, and progress.',
    {
    ...
    withErrorHandling(async ({ idProject, includeDependencies }) => {
      ...
      const ganttData = (activities as Array<Record<string, unknown>>).map((a) => ({
        ...
        startDate: a.plannedStartDate, endDate: a.plannedEndDate,
        progress: a.progress ?? 0, parentId: a.idActivity ?? null,
        dependencies: includeDependencies ? a.dependencyIds ?? [] : undefined,
      }));
      return { content: [{ type: 'text' as const, text: JSON.stringify({ idProject, activities: ganttData }, null, 2) }] };
    }, 'get_gantt_data'),
  );

  server.tool(
    'add_dependency',
    'Add a dependency between two activities. The predecessor activity must be completed before the successor can start.',
    {
    ...
    withErrorHandling(async ({ predecessorId, successorId, dependencyType, lag }) => {
      ...
      return { content: [{ type: 'text' as const, text: `Dependency added successfully:\n${JSON.stringify(result, null, 2)}` }] };
    }, 'add_dependency'),
  );
}
