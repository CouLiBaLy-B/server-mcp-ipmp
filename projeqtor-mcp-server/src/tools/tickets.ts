...
export function registerTicketTools(server: McpServer, client: ProjeqtorApiClient, log: Logger) {
  server.tool(
    'list_tickets',
    ...
    withErrorHandling(async ({ idProject, status, priority, responsibleId, type }) => {
      ...
      const tickets = await client.getAll('Ticket', Object.keys(params).length > 0 ? params : undefined);
      return { content: [{ type: 'text' as const, text: JSON.stringify({ count: (tickets as Array<unknown>).length, tickets }, null, 2) }] };
    }, 'list_tickets'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async (args) => {
      ...
      return { content: [{ type: 'text' as const, text: `Ticket created successfully:\n${JSON.stringify(result, null, 2)}` }] };
    }, 'create_ticket'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async (args) => {
      ...
      return { content: [{ type: 'text' as const, text: `Ticket updated successfully:\n${JSON.stringify(result, null, 2)}` }] };
    }, 'update_ticket'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async ({ idProject, textSearch, status, priority, responsibleId, dateFrom, dateTo }) => {
      ...
      if (dateTo) params.creationDate = params.creationDate ? params.creationDate + ',<=' + dateTo.replace(/-/g, '') : '<=' + dateTo.replace(/-/g, '');
      let tickets = await client.getAll('Ticket', Object.keys(params).length > 0 ? params : undefined);
      if (textSearch) {
        const lower = textSearch.toLowerCase();
        tickets = (tickets as Array<Record<string, unknown>>).filter((t) =>
          String(t.name ?? '').toLowerCase().includes(lower) || String(t.description ?? '').toLowerCase().includes(lower),
        );
      }
      return { content: [{ type: 'text' as const, text: JSON.stringify({ count: (tickets as Array<unknown>).length, tickets }, null, 2) }] };
    }, 'search_tickets'),
