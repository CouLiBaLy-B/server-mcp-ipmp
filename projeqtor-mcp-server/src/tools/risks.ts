...
export function registerRiskTools(server: McpServer, client: ProjeqtorApiClient, log: Logger) {
  server.tool(
    'list_risks',
    'List all risks for a project. Each risk has a probability and impact score, allowing you to calculate risk exposure (probability x impact). Filter by status to see only active risks.',
    {
    ...
    withErrorHandling(async ({ idProject, status }) => {
      ...
      const enriched = (risks as Array<Record<string, unknown>>).map((r) => ({
        ...r, exposure: (Number(r.probability ?? 0) * Number(r.impact ?? 0)),
      }));
      enriched.sort((a, b) => b.exposure - a.exposure);
      return { content: [{ type: 'text' as const, text: JSON.stringify({ count: enriched.length, risks: enriched }, null, 2) }] };
    }, 'list_risks'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async (args) => {
      ...
      return { content: [{ type: 'text' as const, text: `Risk created successfully:\n${JSON.stringify(result, null, 2)}` }] };
    }, 'create_risk'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async (args) => {
      ...
      return { content: [{ type: 'text' as const, text: `Risk updated successfully:\n${JSON.stringify(result, null, 2)}` }] };
    }, 'update_risk'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async ({ idProject, status, priority, responsibleId }) => {
      ...
      return { content: [{ type: 'text' as const, text: JSON.stringify({ count: (issues as Array<unknown>).length, issues }, null, 2) }] };
    }, 'list_issues'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async (args) => {
      ...
      return { content: [{ type: 'text' as const, text: `Issue created successfully:\n${JSON.stringify(result, null, 2)}` }] };
    }, 'create_issue'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async ({ idProject, status }) => {
      ...
      return { content: [{ type: 'text' as const, text: JSON.stringify({ count: (opportunities as Array<unknown>).length, opportunities }, null, 2) }] };
    }, 'list_opportunities'),
  );
}
