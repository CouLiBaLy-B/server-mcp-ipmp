...
export function registerPlanningTools(server: McpServer, client: ProjeqtorApiClient, log: Logger) {
  server.tool(
    'log_work',
    ...
    withErrorHandling(async (args) => {
      ...
      return { content: [{ type: 'text' as const, text: `Time logged successfully:\n${JSON.stringify(result, null, 2)}` }] };
    }, 'log_work'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async ({ idResource, startDate, endDate }) => {
      const days = daysBetween(startDate.replace(/-/g, '') + '000000', endDate.replace(/-/g, '') + '235959');
      ...
      return {
        content: [{
          type: 'text' as const,
          text: JSON.stringify({ idResource, period: { startDate, endDate, totalDays: days }, entries: wk, totalHours: wk.reduce((sum, w) => sum + Number(w.work ?? 0), 0) }, null, 2),
        }],
      };
    }, 'get_timesheet'),
  );
  ...
  server.tool(
    'get_planning',
    'Get the planning/schedule for a project. Returns all activities with their planned dates, actual dates, progress, and dependencies.',
    {
    ...
    withErrorHandling(async ({ idProject, includeClosed }) => {
      ...
      const planning = filtered.map((a) => ({
        ...
        plannedStart: a.plannedStartDate, plannedEnd: a.plannedEndDate,
        realStart: a.realStartDate, realEnd: a.realEndDate,
        progress: a.progress ?? 0, status: a.status,
        responsibleId: a.responsibleId, parentId: a.parentId,
      }));
      return { content: [{ type: 'text' as const, text: JSON.stringify({ idProject, activities: planning }, null, 2) }] };
    }, 'get_planning'),
  );
}
