...
import { expenseSchema, budgetSchema } from '../schemas/projeqtor-types.js';
...
export function registerFinancialTools(
  ...
  server.tool(
    'get_project_budget',
    'Get budget information for a project. Returns planned budget, actual costs, remaining budget, and variance analysis. Useful for financial tracking and reporting.',
    {
      idProject: z.string().describe('Project ID to get budget for'),
    },
    withErrorHandling(async ({ idProject }) => {
      // Get the project details first
      const project = await client.getById('Project', idProject);
      const budgets = await client.getAll('Budget', { idProject });
      const expenses = await client.getAll('Expense', { idProject });

      const totalBudget = (budgets as Array<Record<string, unknown>>).reduce(
        (sum, b) => sum + Number(b.amount ?? 0),
        0,
      );
      const totalExpenses = (expenses as Array<Record<string, unknown>>).reduce(
  ...
  server.tool(
    ...
    withErrorHandling(async (args) => {
      ...
      return {
        content: [{ type: 'text' as const, text: `Expense recorded successfully:\n${JSON.stringify(result, null, 2)}` }],
      };
    }, 'create_expense'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async ({ idProject, dateFrom, dateTo, type, idResource }) => {
      ...
      const total = (expenses as Array<Record<string, unknown>>).reduce(
        (sum, e) => sum + Number(e.amount ?? 0),
        0,
      );
      ...
      return {
        content: [
          {
            type: 'text' as const,
            text: JSON.stringify({ count: (expenses as Array<unknown>).length, total, expenses }, null, 2),
          },
        ],
      };
    }, 'list_expenses'),
  ...
  server.tool(
    ...
    withErrorHandling(async ({ idProject }) => {
      ...
      const budgets = await client.getAll('Budget', { idProject });
      const expenses = await client.getAll('Expense', { idProject });
      const works = await client.getAll('Work', { idProject });
      ...
      const totalWorkHours = (works as Array<Record<string, unknown>>).reduce(
