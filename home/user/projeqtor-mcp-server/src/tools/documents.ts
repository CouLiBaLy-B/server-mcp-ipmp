...
// ─── Meetings, Documents & Decisions Tools ─────────────────────────────────────

export function registerDocumentMeetingTools(
  server: McpServer,
    ...
    withErrorHandling(async ({ idProject, status, dateFrom, dateTo }) => {
      ...
      return {
        content: [
          {
            type: 'text' as const,
            text: JSON.stringify({ count: (meetings as Array<unknown>).length, meetings }, null, 2),
          },
        ],
      };
    }, 'list_meetings'),
  ...
  server.tool(
    ...
    withErrorHandling(async (args) => {
      ...
      return {
        content: [{ type: 'text' as const, text: `Meeting created successfully:\n${JSON.stringify(result, null, 2)}` }],
      };
    }, 'create_meeting'),
  );
  ...
  server.tool(
    ...
    withErrorHandling(async ({ idProject, type, nameFilter }) => {
      ...
      if (nameFilter) {
        ...
        docs = (docs as Array<Record<string, unknown>>).filter(
          (d) => String(d.name ?? '').toLowerCase().includes(lower),
        );
      }
      ...
      return {
        content: [
          {
            type: 'text' as const,
            text: JSON.stringify({ count: (docs as Array<unknown>).length, documents: docs }, null, 2),
          },
        ],
      };
    }, 'list_documents'),
    ...
    withErrorHandling(async ({ idProject, status }) => {
      ...
      return {
        content: [
          {
            type: 'text' as const,
            text: JSON.stringify({ count: (decisions as Array<unknown>).length, decisions }, null, 2),
          },
        ],
      };
    }, 'list_decisions'),
  ...
  server.tool(
    'list_questions',
    'List all open questions for a project. Questions are items raised during project execution that need answers or clarification. Filter by status to see unanswered ones.',
    {
    ...
    withErrorHandling(async ({ idProject, status }) => {
      ...
      return {
        content: [
          {
            type: 'text' as const,
            text: JSON.stringify({ count: (questions as Array<unknown>).length, questions }, null, 2),
          },
        ],
      };
    }, 'list_questions'),
