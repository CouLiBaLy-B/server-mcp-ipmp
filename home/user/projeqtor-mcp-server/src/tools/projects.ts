...
export function registerProjectTools(
  ...
  server.tool(
    ...
    withErrorHandling(async ({ status, type, nameFilter }) => {
      ...
      if (nameFilter) {
        ...
        projects = (projects as Array<Record<string, unknown>>).filter(
  ...
  server.tool(
    ...
    {
      id: z.string().describe('The unique project ID (e.g., "1", "42")'),
    },
  ...
  server.tool(
    ...
    'Create a new project in ProjeQtOr. Provide at minimum a name. Optional fields include description, type, dates, budget, and client. All dates must be in YYYY-MM-DD format.',
    {
      ...
      type: z.string().optional().describe('Project type (e.g., "1", "2")'),
  ...
  server.tool(
    ...
    withErrorHandling(async (args) => {
      const { id, ...rest } = args;
      const data = { id, ...rest };
      const result = await client.update('Project', data);
      return {
  ...
  server.tool(
    ...
    withErrorHandling(async ({ id }) => {
      ...
      const kpis = {
        ...
        realStartDate: p.realStartDate ?? null,
        realEndDate: p.realEndDate ?? null,
        plannedWork: p.plannedWork ?? 0,
        realWork: p.realWork ?? 0,
        remainingWork: p.remainingWork ?? 0,
        plannedCost: p.budget ?? p.plannedCost ?? 0,
        realCost: p.realCost ?? 0,
