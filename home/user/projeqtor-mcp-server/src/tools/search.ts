...
import { daysAgo, now as nowPq } from '../utils/date-utils.js';

// ─── Search & Global Tools ────────────────────────────────────────────────────

/** All object classes searchable in ProjeQtOr */
    ...
    withErrorHandling(async ({ searchTerm, classes, idProject }) => {
      ...
      await Promise.all(
        targets.map(async (objectClass) => {
          try {
            ...
            if (matched.length > 0) {
              results[objectClass] = matched;
            }
          } catch {
            // Skip classes that fail (not enabled/accessible)
          }
        }),
      );
      ...
      return {
        content: [
          {
            type: 'text' as const,
            text: JSON.stringify({ searchTerm, totalHits, results }, null, 2),
          },
        ],
      };
    }, 'global_search'),
      ...
      await Promise.all(
        targets.map(async (cls) => {
          try {
            ...
            if ((items as Array<unknown>).length > 0) {
              results[cls] = items as unknown[];
            }
          } catch {
        ...
        content: [
          {
            ...
            text: JSON.stringify(
              {
                period: { from, to, daysBack: Math.min(daysBack, 365) },
                totalChanges,
                changes: results,
              },
              null,
